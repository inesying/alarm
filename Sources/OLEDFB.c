...
void OLEDFB_clear()
{
	uint32* tmp = (uint32*)(&OLEDFB_frameBuffer);
	uint16 i;
	for(i = 0;i<=0xFF;i++)
	{
		tmp[i] = 0;
...
{
    int16 deltaX = x1>=x2?(x1-x2):(x2-x1);
    int16 deltaY = y1>=y2?(y1-y2):(y2-y1);
    if (deltaX > deltaY)
    {
        if(x1 > x2)
        {
		...
		uint16 x,y;
		for(x = x1, y = y1; x <= x2 && x<128/* && y >= 0 && y<64*/; x++, error-=deltaY)
        {
            if(error<0&&deltaY>0)
            {
                y++;
    ...
    else
    {
		...
		uint16 x,y;
		for(y = y1, x = x1; y <= y2 && y <64/* && x>=0 && x<128*/; y++, error-=deltaX)
        {
            if(error<0&&deltaX>0)
            {
                x++;
